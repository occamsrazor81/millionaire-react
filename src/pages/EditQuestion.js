...
const EditQuestion = () => {
  ...
  React.useEffect(() => {
    const singleMill = millionaire.filter(
      (single) => single.id === parseInt(id)
    );
    newQuestionDispatch({ type: 'EDIT_QUESTION', payload: singleMill[0] });
  }, [id, newQuestionDispatch, millionaire]);

  React.useEffect(() => {
    if (newQuestionState.flag) {
      dispatch({
        type: 'SAVE_EDITED_QUESTION',
        payload: { vars: newQuestionState, id: id },
      });
